...
**Signal faible et diffus.** L'analyse d'explicabilité (SHAP) montre qu'aucune variable ne domine la décision du modèle : les 5 variables les plus influentes n'expliquent qu'une partie modérée de l'impact total, et il faut regrouper une dizaine de variables pour couvrir environ 57% de l'impact (temp_mean_24h, pressure_mean_24h, voltage_mean_24h, entre autres). Autrement dit, le modèle combine de nombreux indices faibles plutôt que de s'appuyer sur un signal fort et unique — un profil cohérent avec la nature progressive d'une dégradation mécanique, mais qui limite la performance atteignable.

**Faux négatifs (pannes manquées).** Au seuil de décision retenu, le modèle détecte environ 29% des vraies pannes sur le jeu de test — ce qui signifie qu'environ 71% des pannes réelles à 24h **ne sont pas détectées** par le modèle. C'est la limitation la plus importante à communiquer à tout utilisateur : ce modèle réduit le risque, il ne l'élimine pas.

**Faux positifs (fausses alertes).** À l'inverse, environ 5% des machines saines déclenchent une alerte à tort à ce même seuil — un coût opérationnel (temps d'inspection sans panne trouvée) à mettre en balance avec le coût d'une panne manquée.
...
alerte = proba > 0.4042  # seuil calibré, voir section Evaluation
```
...
Entraînement du modèle final : 7.26 secondes sur un seul cœur de processeur (0.000075 kWh consommés). Les hyperparamètres eux-mêmes ont été obtenus via une recherche Optuna antérieure (20 essais, ~3 minutes) — non refaite ici, réutilisée telle quelle (voir `optuna_xgboost_predictive_maintenance.ipynb`).
...
| PR-AUC | 0.4488 |
| AUC-ROC | 0.6811 |
| Seuil de décision | 0.4042 (percentile 95 des scores sur machines saines de validation) |
| Rappel (pannes détectées) | 29.0% |
| Précision (alertes confirmées) | 55.0% |
| Faux positifs (fausses alertes) | 4.9% |


#### Summary

Le modèle détecte environ 29% des pannes réelles à 24h, avec un taux de fausses alertes d'environ 5% sur les machines saines, au seuil retenu. La performance est modeste mais honnête au regard du signal disponible (voir Model Examination) — un compromis assumé plutôt qu'une sur-promesse.
...
- **Variables dominantes** : temp_mean_24h, pressure_mean_24h, voltage_mean_24h, rotation_max_24h, temp_max_24h — des agrégats sur 24h de température, tension et rotation, cohérents avec une dégradation mécanique progressive plutôt qu'un pic isolé.
- **Concentration de l'impact** : diffus plutôt que concentré — les 10 premières variables couvrent environ 57% de l'impact total sur les prédictions, aucune variable ne domine à elle seule.
...
- **Hardware Type:** CPU — 4 cœur(s) détecté(s) et utilisé(s) pour l'entraînement (pas de GPU)
- **Hours used:** 0.00202 heures (7.26 secondes) pour l'entraînement du modèle final
...
- **Carbon Emitted:** 0.0042 grammes de CO2 équivalent (mesuré avec CodeCarbon, 0.000075 kWh consommés)
...
Poste de calcul standard (CPU, 4 cœur(s) disponibles et utilisés), pas d'infrastructure distribuée ni de GPU nécessaire pour ce type de modèle.
...
@misc{predictive_maintenance_xgboost_24h_v1.0.0_5f2e2b2a,
  title = {AELION TRAINNING - Predictive Maintenance XGBoost Model (24h horizon)},
  author = {[Haja RAKOTOVOALAVO PETERA]},
  year = {2026},
...
[More Information Needed] (2026). AELION TRAINNING - Predictive Maintenance XGBoost Model (24h horizon), predictive-maintenance-xgboost-24h-v1.0.0-5f2e2b2a.
...
Voir les notebooks associés du projet : `pipeline_optuna_xgboost_predictive_maintenance.ipynb` (recherche d'hyperparamètres), `pipeline_eco_conception_codecarbon.ipynb` (mesure d'impact comparée ML/DL), `pipeline_shap_explainability_ml.ipynb` (explicabilité détaillée).
